...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
    ...
    {
        if (cmd_app(cons, fat, cmdline) == 0) /*不是命令?��也不是空�?*/
        {
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
    ...
    name[i] = 0; /*暂且�?�?件名的后面置为0*/

    /*寻找�?件 */
    finfo = file_search(name, (struct FILEINFO *)(ADR_DISKIMG + 0x002600), 224);
    if (finfo == 0 && name[i - 1] != '.')
    {
        /*由于找不到�?件?��故在�?件名后面�?上�?.hrb”后重新寻找*/
        name[i] = '.';
        name[i + 1] = 'H';
        name[i + 2] = 'R';
        name[i + 3] = 'B';
    ...
    {
        /*找到文件情况*/
        p = (char *)memman_alloc_4k(memman, finfo->size); /* 申请代码段内存空间 */
        ...
        if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00)
        {
            ...
            datsiz = *((int *)(p + 0x0010));
            dathrb = *((int *)(p + 0x0014));
            q = (char *)memman_alloc_4k(memman, segsiz); /* 申请数据段内存空间 */
            task->ds_base = (int)q;
...
void cmd_exit(struct CONSOLE *cons, int *fat)
{
    ...
    if (cons->sht != 0)
    {
        fifo32_put(fifo, cons->sht - shtctl->sheets0 + 768); /* 768到1023 */
    }
    else
    {
        /* taskctl已定义在头文件 */
        fifo32_put(fifo, task - taskctl->tasks0 + 1024); /*1024到2023*/
    }
    io_sti();
    for (;;)
    {
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
    ...
    else if (edx == 5) /* 新建窗口api */
    {
        ...
        sheet_slide(sht, ((shtctl->xsize - esi) / 2) & ~3, (shtctl->ysize - edi) / 2); /* 使显示窗口�?起始位置的X坐标为4�?倍数 */
        sheet_updown(sht, shtctl->top);                                                /*�?窗口图层高度�?定为当前�?�?所在图层的高度?���?�?移到上�?*/
        reg[7] = (int)sht;
    }
    ...
    else if (edx == 16) /* 申请定时器 */
    {
        ...
        ((struct TIMER *)reg[7])->flags2 = 1; /* 允许自动取�? */
    }
    ...
    else if (edx == 27) /* 获取当前languagemode */
    {
        reg[7] = task->langmode;
    }
    return 0;
