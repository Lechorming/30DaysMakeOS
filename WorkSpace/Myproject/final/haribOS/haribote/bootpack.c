...
void HariMain(void)
{
	...
	*((int *)0x0fec) = (int)&fifo;		 /* 将缓冲区的地址放在一个固定地址里 */
	init_pit();							 /* 初始化可编程间隔型定时器PIT */
	...
	memman_free(memman, 0x00400000, memtotal - 0x00400000); /* 4MB-memtotal */
	...
	*((int *)0x0fe4) = (int)shtctl;
	...
	/*载入chinese.fnt */
	fat = (int *)memman_alloc_4k(memman, 4 * 8640);
	...
	{
		/* 全角字符32字节一个, 一个区94位, chinese.fnt字库中除去半角字符部分共94个区 */
		chinese = (unsigned char *)memman_alloc_4k(memman, 16 * 256 + 32 * 94 * 94);
		/* 半角字符16字节一个 */
		for (i = 0; i < 16 * 256; i++)
		{
			chinese[i] = hankaku[i]; /*半角部分直接复制英文字库*/
		}
	...
	*((int *)0x0fe8) = (int)chinese;
	memman_free_4k(memman, (int)fat, 4 * 8640);
