...
; 画面设定
VBEMODE EQU		0x0105			; VBE(VESA-BIOS扩展)的1024*768*8位色彩画面模式
; (画面模式一览)
;	0x100 :  640 *  400 * 8bit
;	0x101 :  640 *  480 * 8bit
;	0x103 :  800 *  600 * 8bit
...
; BOOT_INFO的地址信息
CYLS	EQU		0x0ff0			; IPL加载了多少个柱面到内存0x8200中的所在地址
LEDS	EQU		0x0ff1			; 键盘的LED灯状态的地址
VMODE	EQU		0x0ff2			; 颜色位数的所在地址
SCRNX	EQU		0x0ff4			; 分辨率的X的所在地址
SCRNY	EQU		0x0ff6			; 分辨率的Y的所在地址
VRAM	EQU		0x0ff8			; 图像缓冲区的开始地址

		ORG		0xc200			; 告诉编译器程序在内存中的起始地址，后面要取标号的时候编译器会加上该偏移量
...
; 检查是否兼容VBE
		...
		INT		0x10
		CMP		AX,0x004f
		JNE		scrn320			; 不兼容只能用320*200*8bit画面模式
...
; 取得画面模式信息
        ...
        MOV     AX,0x4f01		; 使用VBE
        INT     0x10			; 调用显卡中断
        CMP     AX,0x004f		; 检查结果是否为0x004f
        JNE     scrn320			; 不等于则只能使用320p

; 画面模式信息的确认
        CMP      BYTE [ES:DI+0x19],8	; 检查颜色数是否为8
        JNE      scrn320
        CMP      BYTE [ES:DI+0x1b],4	; 检查是否为调色板模式
        JNE      scrn320
        MOV      AX, [ES:DI+0x00]		; 检查画面模式号码可否加上0x4000再进行指定
        AND      AX,0x0080
        JZ       scrn320				; 模式属性的bit7是0的话就放弃
...
; 画面模式的切换
		...
		MOV		AX, [ES:DI+0x12]
		MOV		[SCRNX], AX			; 记录分辨率x到bootinfo
		MOV		AX, [ES:DI+0x14]
		MOV		[SCRNY], AX			; 记录分辨率y到bootinfo
		MOV		EAX, [ES:DI+0x28]
		MOV		[VRAM], EAX			; 记录VRAM的内存地址到bootinfo
		JMP		keystatus			; 跳过下面的scrn320
...
; 将IPL读进来在内存0x8000位置的数据复制到内存0x00100000位置去，长度360KB
...
; 所有剩下的
; 将0x00008200地址的数据复制到0x00100200地址处，长度为360KB-512B
...
memcpy:
		...
		RET
; memcpy只要不忘记放入地址大小前缀，也可以用作复制字符串指令
