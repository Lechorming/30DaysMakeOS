...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
    ...
    {
        if (cmd_app(cons, fat, cmdline) == 0) /*不是命令但也不是空字符串*/
        {
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
    ...
    else if (edx == 5) /* 新建窗口api */
    {
        ...
        sheet_updown(sht, shtctl->top);                                                /*将窗口图层高度设定为当前的所在图层的高度并移到最上层*/
        reg[7] = (int)sht;
    }
    ...
    else if (edx == 11) /* 在窗口中画点 */
    {
        ...
        hrb_api_dotwin(sht, esi, edi, eax);
    ...
    else if (edx == 15) /* 等待键盘按键或定时器的API */
    {
        for (;;)
        {
            ...
            if (i == 4) /* 打开命令行 */
            {
                timer_cancel(cons->timer);
                io_cli();
                fifo32_put(sys_fifo, cons->sht - shtctl->sheets0 + 2024); /* 2024到2279 */
                cons->sht = 0;
                io_sti();
            }
            if (256 <= i) /*键盘数据或定时器数据，通过任务A获得*/
            {
                reg[7] = i - 256;
                return 0;
            }
